...
const Purchase = () => {
  ...
  const proceedToPurchaseMemo = useCallback(proceedToPurchase, [
    proceedToPurchase,
  ]);
      ...
      <div className="bg-[#F5F5F5]">
        ...
        <div className="flex items-center justify-between mt-20 mb-5">
          ...
          <div className="rounded-full mr-20">
            <button onClick={toggleCartMemo} className="relative">
              {" "}
              <TiShoppingCart className="text-5xl text-blue-600 active:scale-95 transition-transform duration-75" />
              {cart.length > 0 && (
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          <table className="min-w-full divide-gray-200">
            <thead className="bg-[#595959] text-white">
              <tr>
                <th className="text-center font-medium uppercase tracking-wider py-2">
                  Product name
                </th>
                <th className="text-center font-medium uppercase tracking-wider py-2">
                  Price
                ...
                >
                  <main className=" bg-[#F5F5F5] p-6 shadow-2xl border-2 border-white/20 w-xl max-h-[23rem] overflow-auto">
                    <div className="flex align-center justify-between mb-4">
                      <h2 className="text-xlg font-semibold text-[#00AEEF]">
                        Cart
                      </h2>
                      <button
                        onClick={toggleCartMemo}
                        className="text-lg text-[#1A1A1A] hover:text-blue-600 fixed ml-[30rem]"
                      >
                        <IoMdCloseCircle />
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col gap-2.5">
                      {cart.map((item, index) => (
                        <section
                          key={index}
                          className={`bg-amber-200 rounded-sm p-2`}
                        >
                          <span className="flex items-center justify-between p-1">
                            <li className="font-semibold text-[#1A1A1A]">{item.name}</li>
                            <button
                          ...
                          <span className="flex items-center justify-between p-1">
                            <li className="text-semibold text-[#1A1A1A]">₱ {item.price} </li>
                    ...
                    <button onClick={proceedToPurchaseMemo}>
                      Proceed to Purchase
                    </button>
                  </main>
                </motion.div>
...
const ProductItem = ({ product, navigate, addToCart, index }) => (
  <tr className={`${index % 2 === 0 ? "bg-blue-100 " : ""} hover:bg-red-100`}>
    <td className="text-center py-2 whitespace-nowrap font-semibold">
      {product.name}
    </td>
    <td className="text-center py-2 whitespace-nowrap font-semibold">
      ₱ {product.price}
    </td>
    <td className="py-2 text-center whitespace-nowrap font-semibold">
      {product.quantity}
    </td>
